...
2. For a window, move the pointer to preview the live highlight. As soon as Brushot detects a capturable window, its real application icon, application name, and actual pixel size appear in the center; document names and browser page titles are not used as the application name. Click only to lock the current window, then choose `Start Recording Video` or `Start Recording GIF`. Video can capture system audio, a selected built-in/USB/Bluetooth microphone, or both; GIF is always silent.
3. A centered 3–2–1 countdown appears before capture starts and can be cancelled. During recording, drag the compact floating control bar anywhere within the screen, or use it to pause, resume, stop, cancel, and collapse/expand the annotation tools. Full-screen recordings start with the controls collapsed so less of the screen is covered. The real-time annotation bar provides select, rectangle, arrow, pen, color, stroke width, undo, and redo controls; annotations enter both video and GIF output while the countdown, red border, and controls stay excluded.
